...
[Am,P,xkp1] = init_dmd(full_state_noize);
...
new_state_noize = zeros(4,n_ctrl +1);
new_state_noize(:,1) = full_state_noize(:,end);

uk = -K*full_state_noize(:,end);
...
for i = 1:n_ctrl
    ...
    update_noize = update + randn(size(update))*std;
    new_state(:,i+1) = update;

    [At,Pt,xkp1t] = online_dmd_update(Am,P,xkp1,update_noize,uk);
    K = dlqr(Am,disc_B,Q,R);
